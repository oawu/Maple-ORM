...
// created_at  title
//             created_at
\M\Model::setCaseTable(\M\Model::CASE_SNAKE);
\M\Model::setCaseColumn(\M\Model::CASE_SNAKE);
```
...
```php
\M\Model::setQueryLogFunc(static function(string $db, string $sql, array $vals, string $status, float $during) {
});
```

若要取得上次的 Log，可以使用 `\M\Model::getLastQueryLog()` 方法。
...
若要取得上次的 Log，可以使用 `\M\Model::getLastLog()` 方法。
...
```php
\M\Model::setCacheFunc('MetaData', fn (string $key, callable $closure) {
  return $closure();
});
```
...
```php
\M\Model::setImageThumbnail(fn ($file) => \M\Core\Thumbnail\Imagick::create($file));
```
