...
final class Gd extends Thumbnail {
  public static function getImage($format, $path) {
    switch ($format) {
      ...
      default: throw new \Exception('找尋不到符合的格式，或者不支援此檔案格式！格式：' . $format);
    }
  }
  public static function block9(array $files, string $path, ?bool $interlace = true, ?int $quality = null): bool {
    ...
    switch (pathinfo($path, PATHINFO_EXTENSION)) {
      case 'jpg':          return imagejpeg($image, $path, $quality !== null ? $quality : -1);
      case 'gif':          return imagegif($image, $path);
      case 'webp':         return imagewebp($image, $path, $quality !== null ? $quality : -1);
      case 'png': default: return imagepng($image, $path, $quality !== null ? $quality : -1);
    }
    return false;
  }
  public static function photos(array $files, string $path, bool $interlace = true, ?int $quality = null): bool {
    ...
    for ($i = 0; $i < $c; $i++) {
      imagecopymerge(
        $image, self::create($files[$i])->adaptiveResizeQuadrant($positions[$i]['width'], $positions[$i]['height'])->_getImage(),
  ...
  public function save(string $path): bool {
    ...
    switch ($this->getExtension()) {
      case 'jpg': return imagejpeg($image, $path, $this->_options['jpgQuality']);
      case 'gif': return imagegif($image, $path);
      case 'png': return imagepng($image, $path, $this->_options['pngQuality']);
      case 'webp': return imagewebp($image, $path, $this->_options['webpQuality']);
